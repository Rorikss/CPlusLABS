...
void ArgParser::ParseArgument(std::string& arg, Argument<std::string>* &positional_string) {
...
std::vector<std::string> ArgParser::GetStringValues(const std::string& arg) {
    Argument<std::string>* argument = FindStringArgument(arg);
    if (argument != nullptr) {
        return argument->GetArgumentValues();
    }
    return {};
}
...
bool ArgParser::Parse(std::vector<std::string> args) {
    ...
    Argument<std::string>* pos_string_argument = nullptr;
    for (int i = 1; i < args.size(); ++i) {
        // if (pos_string_argument != nullptr) std::cout << pos_string_argument->GetLongName() << "\n";
        ...
        if (line[0] == '-') {
            ...
            ParseArgument(args[i], pos_string_argument);
